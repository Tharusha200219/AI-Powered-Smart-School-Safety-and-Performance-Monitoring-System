...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Subtle hover effect for scroll button
  scrollBtn.addEventListener("mouseenter", function () {
    this.style.transform = "translateY(-2px) scale(1.05)";
    this.style.boxShadow = "0 6px 18px rgba(6, 193, 103, 0.35)";
  });

  scrollBtn.addEventListener("mouseleave", function () {
    this.style.transform =
      window.pageYOffset > 300 ? "translateY(0)" : "translateY(10px)";
    this.style.boxShadow = "0 4px 12px rgba(6, 193, 103, 0.25)";
  });
...
function applyThemePreset(theme) {
  ...
  if (presets[theme]) {
    ...
    document.getElementById("secondary_color").value = presets[theme].secondary;
...
function updateSchoolInfo() {
  ...
  formData.append("school_type", document.getElementById("school_type").value);
  formData.append(
    "school_motto",
...
function updateAcademicSettings() {
  ...
  fetch("/admin/settings/update-academic", {
    ...
    .then((data) => {
      ...
      if (data.success) {
        showNotification("Academic settings updated successfully!", "success");
      } else {
        showNotification(
