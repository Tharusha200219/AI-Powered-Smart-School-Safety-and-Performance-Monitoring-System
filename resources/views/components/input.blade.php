    ...
    <div class="input-group input-group-outline my-1">
        ...
        @elseif ($type === 'file')
            <input type="file" name="{{ $name }}" id="{{ $name }}"
                class="form-control {{ $class ?? '' }} @error($name) is-invalid @enderror" {{ $attr }}
                @if ($isRequired) required @endif
                onchange="previewFile{{ ucfirst($name) }}(event)"
                accept="{{ $accept ?? 'image/*' }}">
            ...
            @if ($showPreview ?? false)
                ...
                <script>
                    function previewFile{{ ucfirst($name) }}(event) {
                        const file = event.target.files[0];
                        const preview = document.getElementById('{{ $name }}-preview');
                        const maxSize = {{ $maxSize ?? 2048 }}; // KB
                        const allowedTypes = '{{ $accept ?? 'image/*' }}'.split(',').map(type => type.trim());
                        ...
                        if (file) {
                            ...
                            reader.onload = function(e) {
                                if ('{{ $accept ?? 'image/*' }}'.includes('image')) {
                                    ...
                                    img.style.cssText = 'max-height: 120px; max-width: 200px; border-radius: 8px; object-fit: contain; box-shadow: 0 2px 8px rgba(0,0,0,0.1);';
                                    preview.appendChild(img);
                                } else {
                                    preview.innerHTML = `<i class="material-symbols-rounded">description</i><p class="mb-0">${file.name}</p>`;
                                }
